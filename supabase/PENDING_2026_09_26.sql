-- Pending 26 September 2026 migrations for the UCAT trainer + planner.
...
-- frontend deploy (function signatures and existing output keys are unchanged).
-- Generated from supabase/migrations on 2026-09-26:
--   20260926120000_admin_trainer_type_breakdown.sql
--   20260926130000_sjt_practice_without_history.sql

begin;

-- ========== 20260926120000_admin_trainer_type_breakdown.sql ==========
-- Admin per-trainer breakdown for EVERY sessions.training_type.
--
-- The admin RPCs below only counted the six original trainers (speed_reading,
-- rapid_recall, keyword_scanning, calculator, inference_trainer, mental_maths),
-- so not_except, unit_conversions and the QR/DM skill trainers (qr_setup,
-- qr_data_extraction, qr_estimation, dm_constraints) never showed on /admin.
--
-- Backwards compatible: every function keeps its signature and jsonb return
-- type, and every existing output key is still returned with the same meaning.
-- New keys only:
--   get_admin_stats(since_ts, until_ts)
--     + sessions_by_type            { <training_type>: sessions }
--   get_admin_usage_summary(since_ts, until_ts)
--     trainer_usage / trainer_questions / trainer_time_seconds now include every
--     training_type present in range (the original eight keys are always present)
--     + users[].sessions_by_type    { <training_type>: sessions }
--   get_admin_new_users(since_ts, until_ts, limit_rows)
--     + [].sessions_by_type
--   get_admin_registrations_overview(limit_rows)
--     + [].sessions_by_type
--     [].trainer_questions / [].trainer_time_seconds now include every training_type
--
-- The breakdowns are generic (grouped by training_type), so a future trainer
-- appears without another migration; the client zero-fills from
-- src/types/training.ts. Bodies were taken from the live definitions
-- (2026-09-26), which differ from the repo copy of get_admin_usage_summary in
-- that guest counts also read public.analytics_events_daily.
--
-- Bug fix: get_admin_usage_summary declared local variables named
-- total_questions / total_time_seconds, which clash with
-- syllogism_sessions.total_questions ("column reference total_questions is
-- ambiguous", 42702, with the default plpgsql.variable_conflict = error), so the
-- live function raised for every admin call and the usage sections of /admin
-- never rendered. The locals are now v_-prefixed; output keys are unchanged.
--
-- Hardening: search_path is now '' (all references are schema-qualified). Admin
-- check (profiles.role = 'admin') is unchanged. Idempotent: create or replace +
-- re-issued grants.
...
begin
  ...
  select coalesce(jsonb_object_agg(t.training_type, t.n), '{}'::jsonb) into by_type
  from (
    select s.training_type, count(*)::int as n
    from public.sessions s
    where s.training_type is not null
      and (since_ts is null or s.created_at >= since_ts)
      and (until_ts is null or s.created_at <= until_ts)
  ...
  select jsonb_build_object(
    ...
    'sessions_speed_reading', coalesce((by_type->>'speed_reading')::int, 0),
    'sessions_rapid_recall', coalesce((by_type->>'rapid_recall')::int, 0),
    'sessions_keyword_scanning', coalesce((by_type->>'keyword_scanning')::int, 0),
    'sessions_calculator', coalesce((by_type->>'calculator')::int, 0),
    'sessions_inference_trainer', coalesce((by_type->>'inference_trainer')::int, 0),
    'sessions_mental_maths', coalesce((by_type->>'mental_maths')::int, 0),
    'sessions_by_type', by_type,
    'syllogism_sessions_count', (
      select count(*)::int
...
revoke all on function public.get_admin_stats(timestamp with time zone, timestamp with time zone) from public, anon;
...
declare
  ...
  users_arr jsonb;
  -- Original eight keys, always present (zero when unused) for older clients.
...
begin
  ...
  -- Per user and trainer (every training_type plus the two syllogism modes).
  user_trainer as (
