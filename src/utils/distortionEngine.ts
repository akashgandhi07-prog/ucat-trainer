/**
 * Question engine for the speed reading / rapid recall (distortion) trainer.
 *
 * Pure logic, deliberately free of React and Supabase imports so that
 * `npm run verify:distortion` can run it over every passage and check the
 * generated statements for the grammatical nonsense students keep reporting
 * ("only all clauses remain", "where they matter all").
 *
 * UK English in all generated copy.
 */
...
/**
 * A swap marked "verb" has verb-only replacements, so it must not fire when the
 * passage used the word as a noun ("the central claim" → "the central argue").
 */
type SynonymEntry = [RegExp, string[]] | [RegExp, string[], "verb"];
...
const SYNONYM_MAP: SynonymEntry[] = [
  ...
  [/\bclaim\b/gi, ["assert", "contend", "maintain", "argue"], "verb"],
  [/\bclaims\b/gi, ["asserts", "contends", "maintains", "argues"], "verb"],
  [/\bclaimed\b/gi, ["asserted", "contended", "maintained", "argued"], "verb"],
  ...
  [/\bmaintain\b(?=\s+that\b)/gi, ["argue", "contend", "assert", "hold"], "verb"],
  [/\bmaintains\b(?=\s+that\b)/gi, ["argues", "contends", "asserts", "holds"], "verb"],
  ...
  [/\bcauses\b/gi, ["leads to", "results in", "triggers", "produces"], "verb"],
  ...
  [/\bsupport\b/gi, ["sustain", "uphold", "back", "endorse"], "verb"],
  [/\bsupports\b/gi, ["sustains", "upholds", "backs", "endorses"], "verb"],
  [/\boppose\b/gi, ["resist", "challenge", "contest", "object to"]],
  [/\bopposes\b/gi, ["resists", "challenges", "contests", "objects to"]],
  [/\bchallenge\b/gi, ["question", "contest", "dispute", "oppose"], "verb"],
  [/\bchallenges\b/gi, ["questions", "contests", "disputes", "opposes"], "verb"],
...
// A determiner, possessive or quantifier followed by up to two modifiers means
// the next word is being used as a noun.
const NOUN_CONTEXT_RE =
  /\b(the|a|an|this|that|these|those|its|their|his|her|our|your|my|no|any|each|every|one|another|such)\s+(\w+\s+){0,2}$/i;
...
function paraphrase(sentence: string): ParaphraseResult {
  ...
  const maxChanges = 2;
  for (const [re, synonyms, wordClass] of shuffle(SYNONYM_MAP)) {
    ...
    if (!match) continue;
    if (wordClass === "verb" && NOUN_CONTEXT_RE.test(result.slice(0, match.index))) continue;
...
// A restrictive adverb immediately before a quantifier fixes its scope, so
// broadening the quantifier produces nonsense rather than a false claim:
// "only a few clauses remain" became "only all clauses remain" (reported
// August 2026). Skip the flip and let another strategy handle the sentence.
// The same applies to a degree adverb sitting in front of a frequency word:
// "two models are most often contrasted" became "most always contrasted".
const RESTRICTIVE_PREFIX_RE = /\b(only|just|merely|barely|solely|most|more|less|least|very|quite|so|too)\s+\W*$/i;
function isScopeRestricted(sentence: string, index: number): boolean {
  return RESTRICTIVE_PREFIX_RE.test(sentence.slice(0, index));
}

/**
 * First match of `re` with a usable index. The phrase tables below are written
 * with /g, which makes String.match return every match and drop `index`, so the
 * negation and restriction guards silently never ran. Search with a non-global
 * clone instead, and replace only that one occurrence: one clean change per
 * statement, which is what the highlighted fragment in the results view assumes.
 */
function findFirstMatch(sentence: string, re: RegExp): { fragment: string; index: number } | null {
  const search = new RegExp(re.source, re.flags.replace("g", ""));
  const match = sentence.match(search);
  if (!match || match.index == null) return null;
  return { fragment: match[0], index: match.index };
}

function replaceAt(sentence: string, index: number, length: number, replacement: string): string {
  return sentence.slice(0, index) + replacement + sentence.slice(index + length);
}

/** Preserve the capitalisation of the word being replaced. */
...
function distortQualifierToAbsolute(s: string): DistortionResult {
  ...
  if (isScopeRestricted(s, match.index)) return { text: s, applied: false };
...
function distortNegation(s: string): DistortionResult {
  ...
  // A clause-final negator carries the whole predicate by ellipsis ("...while
  // banning junk food is not"). Removing it leaves a stub ("...food is ."), so
  // leave those sentences to the other strategies.
  if (/\b(not|never)\b\s*[.,;:]?\s*$/i.test(s)) return { text: s, applied: false };
...
function distortCausal(s: string): DistortionResult {
  ...
  for (const [re, rep, label] of causalPhrases) {
    const m = findFirstMatch(s, re);
    if (!m) continue;
    // "was not partly responsible" → "was not entirely responsible" weakens the
...
/**
 * Reword the untouched part of a statement so True, False and Can't Tell items
 * all read as paraphrases of the passage.
 *
 * Students reported being able to answer without reading: only True statements
 * were reworded, so anything that matched the passage word for word apart from
 * one quantifier was False or Can't Tell, and the injected phrases ("almost
 * never", "more often than not") gave the rest away. `protectedFragment` is the
 * trap itself, which must survive untouched or the item loses its point.
 */
function rewordStatement(text: string, protectedFragment?: string): string {
  const reworded = paraphrase(text).text;
  if (reworded === text) return text;
  if (protectedFragment && !reworded.includes(protectedFragment)) return text;
  if (!isGrammaticallyPlausible(text, reworded)) return text;
  return reworded;
}
...
function mutateSentenceForCantTell(sentence: string): CantTellQuestionParts | null {
  for (const { re, replacement, label } of CANT_TELL_MUTATIONS) {
    ...
    return {
      displayedSentence: rewordStatement(mutated, cased),
...
export type CorrectAnswer = "true" | "false" | "cant_tell";
...
export type AnswerChoice = "true" | "false" | "cant_tell" | null;
/** T/F/CT choice for standard questions; option index (number) for the MC question. */
export type AnswerValue = AnswerChoice | number;
...
export function buildQuestions(passageText: string, count: number, passageTitle?: string): Question[] {
  ...
  for (let i = 0; i < shuffledSentences.length && questions.length < numFalse; i++) {
    ...
    if (result.applied) {
      questions.push({
        kind: "tfct",
        displayedSentence: rewordStatement(result.text, result.replacedFragment),
        correctAnswer: "false",
        passageSnippet: sentence,
        distortionLabel: result.label,
        originalFragment: result.originalFragment,
