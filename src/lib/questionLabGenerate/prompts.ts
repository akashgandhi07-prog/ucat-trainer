...
export function buildAuditMessages(input: {
  ...
  const system = [
    ...
    "Return ONLY JSON:",
    '{ "accuracyPercent": number, "issues": string[] }',
    "",
    "accuracyPercent (0–100) = exam-ready confidence that stem, all four options, correctAnswer, and explanation are fully correct and consistent.",
    "- Use 100 ONLY if you would publish without edits: one clear correct answer, maths and logic verified, traps valid, UK copy clean.",
    "- If ANY calculation, option value, or correctAnswer is wrong, accuracyPercent must be ≤ 40 and list the fix in issues.",
    "- If wording is fixable but maths is right, 70–95 with specific issues.",
    "- Plugin 'not auto-verified' means be cautious: cap at 88 unless you independently verify the answer.",
    "- Do NOT penalise required 'Step 1:', 'Step 2:' explanation structure or the → symbol in working.",
    "- Do NOT flag 'tutorial voice' for Step labels; those are required.",
    "Check UK English, no em/en dash, no AI chat filler, weak distractors, ambiguous stems.",
    "",
...
export function buildRepairMessages(input: {
  ...
  const system = [
    ...
    "- Fix every issue in repairIssues so the question would score accuracyPercent 100 on re-audit.",
