...
export default function ReaderEngine({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const index = Math.floor((elapsedMs / 60000) * wpm);
  ...
  const renderWord = (word: string, i: number) => {
    ...
    } ${isCurrent ? "text-blue-600 scale-105 font-medium" : "text-ucat-body"}`;
  ...
  const paragraphBlock = (paragraphWords: string[], startIndex: number) => (
    <div
      key={startIndex}
      className="flex flex-wrap justify-center gap-x-2 gap-y-1 text-[15px] sm:text-[16px] leading-[1.6] mb-4 last:mb-0 text-ucat-body"
    >
