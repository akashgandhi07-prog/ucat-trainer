    ...
    <div className={cn("pb-4", inAppShell ? APP_CONTENT_X : "px-4")}>
      <div className={cn("w-full pt-2", !inAppShell && "max-w-6xl mx-auto")}>
        ...
        <section className="flex flex-col lg:flex-row gap-3 lg:gap-4">
          ...
          <div className="flex-1 min-w-0 bg-card rounded-xl border border-border p-3 space-y-2">
                ...
                {questions.map((q, index) => {
                  ...
                  return (
                    <li
                      key={q.id}
                      className={
                        "border-b border-border last:border-b-0 pb-2 last:pb-0 " +
                        (showFeedback
                          ? isCorrect
                    ...
                    >
                      <div className="grid grid-cols-[minmax(0,1fr)_auto] lg:grid-cols-[minmax(0,1fr)_6rem] items-center gap-3">
                        ...
                        <p className="text-base text-foreground leading-snug py-0.5">
                          <span className="font-semibold text-muted-foreground">{index + 1}.</span> {q.conclusion_text}
                        </p>
                        ...
                        <div
                          className={
                            ...
                            (userAnswer === true
                              ? "bg-secondary text-foreground w-24"
                              : userAnswer === false
                                ? "bg-secondary text-foreground w-24"
                                : isDropTarget
                            ...
                            <>
                              ...
                              <span className="flex gap-1 lg:hidden">
                                <button
                                  ...
                                  className="min-h-[36px] min-w-[3rem] rounded border border-border bg-white px-2 text-sm font-medium text-foreground hover:border-slate-400 hover:bg-secondary active:bg-secondary touch-manipulation"
                                >
                                  Yes
                                </button>
                                <button
                      ...
                      {showFeedback && (
                        <div className="mt-1.5 pl-0 text-sm">
                          <p className="text-foreground">
                            <span className="font-semibold text-foreground">Correct: {q.is_correct ? "Yes" : "No"}</span>
                            {" · "}
                            <span className={isCorrect ? "text-training-success" : "text-destructive"}>
                              {feedbackLabel}
                            </span>
                          </p>
                          <p className="text-muted-foreground mt-0.5 leading-relaxed">{q.explanation}</p>
                          <button
                            type="button"
                            onClick={() => { setReportingQuestionId(q.id); setFeedbackOpen(true); }}
                            className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-border bg-white/60 px-2.5 py-0.5 text-xs font-medium text-muted-foreground hover:bg-secondary hover:text-foreground"
                          >
            ...
            {questions.length > 0 && (
              <div
                className={
                  "flex flex-shrink-0 flex-col justify-center rounded-lg border-2 border-border bg-secondary p-2 " +
              ...
              >
                <div className="flex flex-col gap-2">
                  <div
                    ...
                    className={
                      "flex min-h-[44px] min-w-[5rem] cursor-grab active:cursor-grabbing items-center justify-center rounded-lg border-2 border-border bg-white px-4 text-base font-medium text-foreground shadow " +
